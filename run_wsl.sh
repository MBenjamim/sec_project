...
tmux new-session -d -s blockchain_network -n server_0 "mvn exec:java -Dexec.mainClass=main.java.server.BlockchainNetworkServer -Dexec.args=\"0 $BASE_PORT_SERVER_TO_SERVER $BASE_PORT_CLIENT_TO_SERVER\""
if [ $? -ne 0 ]; then
